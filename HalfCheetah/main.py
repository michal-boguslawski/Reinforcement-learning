...
T.set_printoptions(precision=4, sci_mode=False, linewidth=500)
np.printoptions(precision=4, suppress=True, linewidth=500)

def reshape_reward(reward, state, next_state, action, factor):
    side_reward = T.zeros_like(reward)
    
    side_reward -= 0.1 * T.square(next_state[..., 1])
...
ac_model = PPO(num_actions=action_dim,
               ...
               backstep=backstep,
               device=device)
memory = Memory(maxlen=memory_timesteps, backstep=backstep*if_rnn)
if if_rnn:
...
while step <= max_timesteps:
    ...
    if done.any():
        # print(f"Step {step}, state {state.mean(0)[:12].cpu().numpy()}, rewards {total_rewards[done].cpu().mean():.4f}, value {output['value'].numpy()}, action {action.mean(0).cpu().numpy()}")
    ...
    if step % train_timesteps == 0 and step > 0:
        ...
        ac_model.update(batch, ppo_epochs=ppo_epochs, backstep=backstep*if_rnn, factor=factor)
