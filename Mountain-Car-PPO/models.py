...
class ActorCritic(nn.Module):
    ...
    def select_action(self, input, hx, temperature: float = 1.):
    ...
    def calculate_loss(self, action, state, hx, old_logprob, advantage, result, factor):
        ...
        loss = policy_loss + 0.5 * critic_loss + self.entropy_reg * factor * entropy
        return loss
...
class PPO:
    ...
    def update(self, sample, ppo_epochs: int = 10, factor: float = 1):
        ...
        for _ in range(ppo_epochs):
            ...
            loss = self.policy.calculate_loss(action, state, hx, old_logprob, advantage, result, factor)
            loss.backward()
